...
const EditorLayout = ({ userData, onLogout }) => {
    ...
    const [initialDuration, setInitialDuration] = useState(7200000); // 2 Hours
    const [metrics, setMetrics] = useState({ time: 7200000 });
    ...
    useEffect(() => {
        const fetchPatterns = async () => {
            ...
            try {
                const res = await fetch(`${import.meta.env.VITE_API_URL}/api/patterns/${userData.category}`);
                ...
                if (Array.isArray(data)) {
                    ...
                    console.error("Patterns data is not an array:", data);
                    setAllPatterns([]);
                }
            } catch (err) {
        ...
        fetchPatterns();
    }, [userData.category, userData.dbPatternsCompleted]);
                ...
                <div className="upper-pane" style={{ flex: 1, display: 'flex', minHeight: '0' }}> {/* flex 1 auto-grows to take available space */}
                    ...
                    <div style={{ width: '40%', display: 'flex', flexDirection: 'column', borderLeft: '1px solid #333' }}>
                        <div style={{ flex: 1, position: 'relative' }}>
                            <PatternView
                                pattern={currentPattern ? currentPattern.target_output : "LOADING..."}
                                levelName={currentPattern ? currentPattern.name : ""}
                                description={currentPattern ? currentPattern.description : ""}
                                challengeType={currentPattern ? currentPattern.challenge_type : "pattern"}
                            />
                        </div>
                    </div>
                </div>
